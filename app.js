(function(){
  ...
  function enhanceNavigation(){
    ...
    if(nav){let contact=[...nav.querySelectorAll('a')].find(a=>a.textContent.trim().toLowerCase()==='contact'||(a.getAttribute('href')||'').startsWith('mailto:'));if(contact){contact.href='contact.html';contact.removeAttribute('target');contact.removeAttribute('rel')}addLink(nav,"Mack's Modules",'macks-modules.html',contact||null);addLink(nav,'Partners','partners.html',contact||null)}
  ...
  function enforceCheckoutOnlyCatalog(){
    ...
    if(productsFooter){
      productsFooter.querySelectorAll('a').forEach(a=>{
        ...
        if(!['rowglass.html','thread-junk-remover.html','macks-modules.html','products.html'].includes(href))a.remove();
      });
    }
  }
  function polishPublicCopy(){
    ...
    if(page==='index.html'){
      ...
      if(hero){
        ...
        if(actions[1]){actions[1].textContent="BOOK MACK'S · $19.99";actions[1].href='macks-modules.html'}
